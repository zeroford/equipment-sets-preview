...
| setKey | slot | level | grade | isMain | isNew | isActive | sub1Type | sub1Value | sub2Type | sub2Value |
|---|---|---|---|---|---|---|---|---|---|---|
| boss | 1 | 97 | 10 | TRUE | FALSE | TRUE | skillAmp | 8.53% | critDmg | 14.62% |
...
- **`isMain`** — `TRUE` = ใบหลักของช่องนั้น เว็บวาดกรอบฟ้าให้ และใช้ใบนี้คิดยอดรวม stat
  กับหน้า Compare — กดปุ่มหมุดบนการ์ดเพื่อย้ายได้ ช่องนึงควรมี `TRUE` ใบเดียว
- **`isNew`** — `TRUE` = จุดแดงมุมขวาบนของรูป ไว้เตือนว่าเพิ่งเพิ่มเข้ามา
  กด Add แล้วติดให้เอง ลบเองในชีตเมื่ออ่านแล้ว (คนละเรื่องกับ `isMain`)
- **`isActive`** — `FALSE` = ของเก่าที่ถูกแทนที่/ลบไปแล้ว ยังอยู่ในชีตแต่ไม่ขึ้นบนเว็บ
  ...
  **ช่องว่างถือว่ายังใช้งานอยู่** ต้องเขียน `FALSE` ชัดๆ เท่านั้นถึงจะถูกซ่อน
  ...
  ⚠️ **ต้องมีคอลัมน์นี้** ไม่งั้นกด Add จะขึ้น error — เพราะถ้าปิดแถวเก่าไม่ได้
  การเพิ่มของจะกลายเป็นเขียนทับของเดิมทิ้ง
